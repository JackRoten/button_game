...
module "networking" {
  ...
  environment  = local.environment
  vpc_cidr     = "10.1.0.0/16"  # Different CIDR from prod
...
module "database" {
  ...
  multi_az             = false  # Gamma can use single-AZ
  
  backup_retention_period = 3  # Shorter retention for gamma
  deletion_protection     = false  # Can delete gamma DB
